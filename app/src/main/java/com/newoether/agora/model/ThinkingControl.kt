...
object ThinkingLevels {
    ...
    val effortValues = listOf("low", "medium", "high", "xhigh", "max")
    ...
    fun effortRangeForProvider(providerName: String?): IntRange = when (providerName?.lowercase()) {
        "openai", "open router" -> 0..3   // Max clamps to xHigh
        "google", "gemini" -> 0..2         // xHigh/Max clamp to High
        else -> 0..4                        // full range (Anthropic, unknown)
    }
    ...
    fun openAiEffort(effort: String): String = when (normalizeEffort(effort)) {
        "none", "low", "medium", "high", "xhigh" -> normalizeEffort(effort)
        "max" -> "xhigh"
        else -> "medium"
    }
